...
#define FACTOR 1//仮の定数、get_extreme_valueで使用、いずれ消去する予定
...
//以下、スマホの磁場マーカからの方位角を検出する処理のグローバル変数
...
int number_of_cnt_ext_max_time = 0;//ノルムが極大値を取った時の時刻の数
...
void azimuth(char *sen_mag_file, char *mag_file, char *norm_file, char *motor_file){
  ...
  while(fscanf(fp_mag,"%lf,%lf,%lf,%lf\n",&f1,&f2,&f3,&f4)!=EOF) {
    if(mag_data_number < MAX_SAMPLE_DATA_NUMBER){
      mag_unixtime[mag_data_number] = f1;
      mag_x[mag_data_number] = f2;
      mag_y[mag_data_number] = f3;
      mag_z[mag_data_number] = f4;
      mag_data_number ++;
...
void elevation(char *sen_mag_file, char *mag_file, char *norm_file, char *motor_file){
  ...
  while(fscanf(fp_mag,"%lf,%lf,%lf,%lf\n", &h_time, &h_x, &h_y, &h_z)!=EOF) {
    ...
    for(l3=0;l3<=ext_max_time_number;l3++){
      //ext_max_timeはノルムが極大値を取っている時の時刻
      if(ext_max_time[l3] - TIME_DELTA <= h_time && h_time <= ext_max_time[l3] + TIME_DELTA){
        //if分の条件はセンサー値が振動を始める前の揺らぎを極値として認識しないための処理、前の処理とかぶってる？
...
void orientation(char *mag_file, char *sen_acc_file, char *mag_file_2){
  ...
  //加速度ベクトルとz軸中心、角度はラジアン
  cos_alpha = ( 1/sqrt(pow(ave_acc_x,2) + pow(ave_acc_y,2)) )*(ave_acc_x);
  ...
  //y軸と加速度ベクトルが成す角度、角度はラジアン
  cos_beta = ( 1/sqrt(pow(ave_acc_y,2) + pow(ave_acc_z,2)) )*(-ave_acc_z);
  beta = acos(cos_beta);
  ...
  rotate_z(mag_x, mag_y, mag_z, mag_data_number, alpha);//z軸を中心に回転させる
  ...
  file_read(&fp_mag_2, mag_file_2);//座標変換した磁気センサのデータ

  while(fscanf(fp_mag_2,"%lf,%lf,%lf,%lf\n", &h_time, &h_x, &h_y, &h_z)!=EOF) {
    //ext_max_timeはノルムが極大値を取っている時の時刻
    if(ext_max_time[5] - TIME_DELTA <= h_time && h_time <= ext_max_time[5] + TIME_DELTA){
      //if分の条件はセンサー値が振動を始める前の揺らぎを極値として認識しないための処理、前の処理とかぶってる？
      h_x_after = h_x;
      h_y_after = h_y;
    }
...
void rotate_x(double *data_x, double* data_y, double* data_z, int data_number, double angle){
  double data_x_after[data_number];
  double data_y_after[data_number];
  double data_z_after[data_number];
  int cnt_data = 0;
  for(cnt_data=0; cnt_data<data_number; cnt_data++){
    data_x_after[cnt_data] = data_x[cnt_data];
    data_y_after[cnt_data] = cos(angle)*data_y[cnt_data] - sin(angle)*data_z[cnt_data];
    data_z_after[cnt_data] = sin(angle)*data_y[cnt_data] + cos(angle)*data_z[cnt_data];
  }
  for(cnt_data=0; cnt_data<data_number; cnt_data++){
    data_x[cnt_data] = data_x_after[cnt_data];
    data_y[cnt_data] = data_y_after[cnt_data];
    data_z[cnt_data] = data_z_after[cnt_data];
  }
}
...
void rotate_y(double *data_x, double* data_y, double* data_z, int data_number, double angle){
  ...
  int cnt_data = 0;
  for(cnt_data=0; cnt_data<data_number; cnt_data++){
    data_x_after[cnt_data] = cos(angle)*data_x[cnt_data] - sin(angle)*data_z[cnt_data];
    data_y_after[cnt_data] = data_y[cnt_data];
    data_z_after[cnt_data] = sin(angle)*data_x[cnt_data] + cos(angle)*data_z[cnt_data];
  }
  for(cnt_data=0; cnt_data<data_number; cnt_data++){
    data_x[cnt_data] = data_x_after[cnt_data];
    data_y[cnt_data] = data_y_after[cnt_data];
...
void rotate_z(double *data_x, double* data_y, double* data_z, int data_number, double angle){
  ...
  int cnt_data = 0;
  for(cnt_data=0; cnt_data<data_number; cnt_data++){
    //printf("%lf %lf %lf\n", data_x[cnt_data], data_y[cnt_data], data_z[cnt_data]);
    data_x_after[cnt_data] = cos(angle)*data_x[cnt_data] - sin(angle)*data_y[cnt_data];
    data_y_after[cnt_data] = sin(angle)*data_x[cnt_data] + cos(angle)*data_y[cnt_data];
    data_z_after[cnt_data] = data_z[cnt_data];
    //printf("%lf %lf %lf\n", data_x_after[cnt_data], data_y_after[cnt_data], data_z_after[cnt_data]);
  }
  for(cnt_data=0; cnt_data<data_number; cnt_data++){
    data_x[cnt_data] = data_x_after[cnt_data];
    data_y[cnt_data] = data_y_after[cnt_data];
